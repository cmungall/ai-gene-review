...
def fetch_gene_data(
    ...
    gene_dir = base_path / "genes" / organism / gene_name
    ...
    uniprot_file = gene_dir / f"{gene_name}-uniprot.txt"
    ...
    if seed_annotations:
        yaml_file = gene_dir / f"{gene_name}-ai-review.yaml"
        ...
        if added_count > 0:
            print(f"  ✓ Seeded {added_count} GOA annotations in {gene_name}-ai-review.yaml")
        else:
            if yaml_existed:
                print(f"  - {gene_name}-ai-review.yaml already contains all GOA annotations")
...
def resolve_organism_code_to_taxon(organism_code: str) -> Optional[str]:
    ...
    try:
        ...
        url = "https://rest.uniprot.org/taxonomy/stream"
        params = {
            "query": f"mnemonic:{organism_code}",
            "format": "json",
            "size": "1"
        }
        response = requests.get(url, params=params, timeout=10)
...
def swissprot_by_gene_taxon(gene_symbol: str, organism: str, limit: int = 10) -> List[Dict[str, str]]:
...
def uniprot_by_gene_taxon(gene_symbol: str, organism: str, limit: int = 10, reviewed_only: bool = True) -> List[Dict[str, str]]:
    ...
    for rec in response.json().get("results", []):
        hits.append({
            ...
            "gene": next((g.get("geneName", {}).get("value") 
                         for g in rec.get("genes", []) if g.get("geneName")), None),
...
def resolve_gene_to_uniprot(gene_name: str, organism: str) -> str:
    ...
    if organism.lower() == "human":
        raise ValueError(f"Could not find Swiss-Prot UniProt ID for gene {gene_name} in {organism}. "
    ...
    if not hits:
        raise ValueError(f"Could not find any UniProt ID (Swiss-Prot or TrEMBL) for gene {gene_name} in {organism}")
...
def fetch_uniprot_data(uniprot_id: str) -> str:
    ...
    if response.status_code != 200:
        raise ValueError(f"Failed to fetch UniProt data for {uniprot_id}: HTTP {response.status_code}")
    ...
    try:
        ...
        sorted_results = sorted(
            ...
            key=lambda x: (
                ...
                -(int(x.get("date", "0")) if x.get("date", "").isdigit() else 0),  # Negative for descending date
                x.get("goId", "")  # Then by GO ID
            )
        )
        
        # Convert JSON to TSV format
        tsv_lines = ["GENE PRODUCT DB\tGENE PRODUCT ID\tSYMBOL\tQUALIFIER\tGO TERM\tGO NAME\tGO ASPECT\tECO ID\tGO EVIDENCE CODE\tREFERENCE\tWITH/FROM\tTAXON ID\tTAXON NAME\tASSIGNED BY\tGENE NAME\tDATE"]
            ...
            if with_from_data:
                for wf in with_from_data:
                    for xref in wf.get("connectedXrefs", []):
                        with_from_list.append(f"{xref.get('db', '')}:{xref.get('id', '')}")
