...
class ValidationIssue(BaseModel):
    ...
    path: Optional[str] = Field(
        default=None,
        description="JSON path or field path where the issue occurred"
    )
...
class BatchValidationReport(BaseModel):
    ...
    reports: List[ValidationReport] = Field(
        default_factory=list,
        description="Individual validation reports for each file"
    )
    timestamp: datetime = Field(
        default_factory=datetime.now,
    ...
    def summary(self) -> str:
        ...
        if self.files_with_warnings > 0:
            ...
            for report in self.get_reports_with_warnings():
                if report.file_path:
                    lines.append(f"  - {report.file_path} ({report.warning_count} warnings)")
