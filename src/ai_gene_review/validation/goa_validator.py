...
class GOAValidationResult:
    ...
    mismatched_labels: List[Tuple[str, str, str]] = field(default_factory=list)  # (go_id, yaml_label, goa_label)
    mismatched_evidence: List[Tuple[str, str, str]] = field(default_factory=list)  # (go_id, yaml_evidence, goa_evidence)
...
class GOAValidator:
            ...
            with open(goa_file, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if not header_skipped:
                        ...
                        if (row[0] in ['DB', 'GENE PRODUCT DB', 'Database'] or 
                            'DB' in row[0] or 
                            'Database' in row[0] or
                            row[0].startswith('DB')):
                            header_skipped = True
                            continue
                        header_skipped = True
    ...
    def validate_against_goa(
        ...
        if not goa_file.exists():
            return GOAValidationResult(
                is_valid=False,
                error_message=f"GOA file not found: {goa_file}"
            )
        ...
        if self.strict_tuple_matching:
            ...
            return self._validate_strict_tuples(goa_annotations, yaml_annotations, result)
        else:
    ...
    def _validate_strict_tuples(
        ...
        for yaml_ann in yaml_annotations:
            ...
            if yaml_tuple not in goa_tuples:
                ...
                partial_matches = [
                    goa_ann for key, goa_ann in goa_by_tuple.items()
                    if key[0] == go_id
                ]
                ...
                if partial_matches:
                    ...
                    goa_evidence_for_term = {ann.evidence_code for ann in partial_matches}
                    if evidence_type and evidence_type not in goa_evidence_for_term:
                        result.mismatched_evidence.append(
                            (go_id, evidence_type, ', '.join(sorted(goa_evidence_for_term)))
                        )
        ...
        for go_id, yaml_anns in yaml_by_go_id.items():
            if go_id not in goa_by_go_id:
                ...
                for yaml_ann in yaml_anns:
                    ...
                    if yaml_evidence and goa_evidence_codes and yaml_evidence not in goa_evidence_codes:
                        # Evidence mismatch
                        result.mismatched_evidence.append(
                            (go_id, yaml_evidence, ', '.join(sorted(goa_evidence_codes)))
                        )
    ...
    def get_summary(self, result: GOAValidationResult) -> str:
        ...
        if result.missing_in_yaml:
            lines.append(f"  - {len(result.missing_in_yaml)} annotations in GOA but not in YAML:")
        ...
        if result.missing_in_goa:
            lines.append(f"  - {len(result.missing_in_goa)} annotations in YAML but not in GOA (exact tuple mismatch):")
        ...
        if result.mismatched_evidence:
            lines.append(f"  - {len(result.mismatched_evidence)} evidence type mismatches:")
    ...
    def seed_missing_annotations(
        ...
        for goa_ann in goa_annotations:
            if goa_ann.reference and (
                goa_ann.reference.startswith('PMID:') or 
                goa_ann.reference.startswith('GO_REF:') or
                goa_ann.reference.startswith('Reactome:')
            ):
        ...
        if pmids_to_add and fetch_titles:
            from ai_gene_review.etl.publication import fetch_pubmed_data, get_cached_title
                    ...
                    if pmid.startswith('PMID:'):
                        ...
                        try:
                            ...
                            if cached_title:
                                ...
                                pub = fetch_pubmed_data(pmid_num, use_cache=True, cache_dir=cache_dir)
    ...
    def _get_reactome_title(self, reactome_id: str, cache: Dict[str, str]) -> str:
        ...
        try:
            ...
            if cached_title:
                ...
                pathway = fetch_reactome_data(clean_id, use_cache=True, cache_dir=cache_dir)
