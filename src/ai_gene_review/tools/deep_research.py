...
@click.option("--model", default="o3-deep-research-2025-06-26", 
...
@click.option("--save-reasoning", is_flag=True, default=False,
              help="Save reasoning steps to debug file (default: off)")
@click.option("--system-prompt-file", type=click.Path(exists=True),
...
def research_gene(
    ...
    """
    if not api_key:
        click.echo("Error: OpenAI API key required. Set OPENAI_API_KEY environment variable or use --api-key option.", err=True)
        sys.exit(1)
...
Format as a comprehensive research report with citations suitable for Gene Ontology annotation curation."""
    ...
    try:
        ...
        if save_reasoning:
            reasoning_steps = [item for item in response.output if hasattr(item, 'type') and item.type == "reasoning"]  # type: ignore
            if reasoning_steps:
