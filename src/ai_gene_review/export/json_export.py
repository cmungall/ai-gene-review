...
class JSONExporter:
    ...
    def export_from_files(self, file_paths: List[Union[str, Path]]) -> List[Dict[str, Any]]:
        """
    ...
    def _flatten_existing_annotations(self, gene_review: GeneReview) -> List[Dict[str, Any]]:
        """
    ...
    def _flatten_annotation(self, annotation: ExistingAnnotation, gene_review: GeneReview) -> Dict[str, Any]:
        ...
        if gene_review.id and annotation.term and annotation.term.id:
            row['id'] = f"{gene_review.id}_{annotation.term.id}_{annotation.evidence_type or 'NA'}"
        else:
        ...
        if annotation.review:
            ...
            row['review.supporting_text'] = review.supporting_text
            ...
            if review.additional_reference_ids:
                row['review.additional_reference_ids'] = '; '.join(review.additional_reference_ids)
            else:
    ...
    def export_to_json(self, file_paths: List[Union[str, Path]], output_path: Union[str, Path]) -> None:
        """
