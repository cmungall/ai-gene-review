...
def create_test_goa_file(annotations: list) -> Path:
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='-goa.tsv', delete=False) as f:
        # Write header
        f.write("DB\tDB_Object_ID\tDB_Object_Symbol\tQualifier\tGO_ID\tGO_Term\tGO_Aspect\tEvidence_Type\tEvidence_Code\tReference\tDate\tAssignedBy\n")
        
        # Write annotations
        for ann in annotations:
            f.write(f"UniProtKB\t{ann['id']}\t{ann['symbol']}\t{ann.get('qualifier', '')}\t"
...
def create_test_yaml_file(gene_symbol: str, annotations: list) -> Path:
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='-ai-review.yaml', delete=False) as f:
        yaml.dump(data, f)
        return Path(f.name)
...
def test_exact_tuple_match_passes():
    ...
    try:
        ...
        assert result.is_valid, f"Should be valid but got: {validator.get_summary(result)}"
