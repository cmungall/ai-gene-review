...
def test_molecular_function_must_be_mf_branch():
    ...
    try:
        ...
        branch_errors = [
            ...
            and "biological_process branch but should be in the molecular_function branch" in issue.message
        ]
...
def test_directly_involved_in_must_be_bp_branch():
    ...
    try:
        ...
        branch_errors = [
            issue for issue in report.issues
            if issue.severity == ValidationSeverity.ERROR
            and "molecular_function branch but should be in the biological_process branch" in issue.message
        ]
        assert len(branch_errors) == 1
        assert not report.is_valid
    finally:
...
def test_locations_must_be_cc_branch():
    ...
    try:
        ...
        branch_errors = [
            issue for issue in report.issues
            if issue.severity == ValidationSeverity.ERROR
            and "biological_process branch but should be in the cellular_component branch" in issue.message
        ]
        assert len(branch_errors) == 1
        assert not report.is_valid
    finally:
