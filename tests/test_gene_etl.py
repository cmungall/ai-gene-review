...
@pytest.mark.parametrize("organism,gene,uniprot_id", [
    ("human", "CFAP300", "Q9BRQ4"),
    ("human", "TP53", "P04637"),
    ("human", "BRCA1", "P38398"),
])
def test_fetch_gene_data_creates_correct_directory_structure(organism, gene, uniprot_id):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert uniprot_id in uniprot_content, f"UniProt ID {uniprot_id} not found in content"
...
def test_fetch_gene_data_handles_invalid_uniprot_id():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with pytest.raises(ValueError, match="Failed to fetch UniProt data"):
            fetch_gene_data(("human", "FAKEGENE"), uniprot_id="INVALID123", base_path=base_path)


@pytest.mark.integration
@pytest.mark.parametrize("organism,gene", [
