...
def sample_yaml_label_mismatch():
    ...
    data = {
        ...
        'existing_annotations': [
            {
                'term': {'id': 'GO:0005515', 'label': 'wrong label'},  # Should be 'protein binding'
                'evidence_type': 'IPI',
                'original_reference_id': 'PMID:29727692'
            },
            {
...
def test_parse_goa_annotation():
    ...
    row = [
        'UniProtKB', 'Q9BRQ4', 'CFAP300', '', 'GO:0005515', 
        'protein binding', 'MF', 'ECO:0000353', 'IPI',
        'PMID:29727692', 'UniProtKB:Q9NVR5', 'NCBITaxon:9606', 
        'Homo sapiens', 'UniProt', 'GO_REF:0000024', '20180515'
    ]
...
def test_empty_goa_file():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as goa_file:
        goa_file.write("GENE PRODUCT DB	GENE PRODUCT ID	SYMBOL	QUALIFIER	GO TERM	GO NAME	GO ASPECT	ECO ID	GO EVIDENCE CODE	REFERENCE	WITH/FROM	TAXON ID	TAXON NAME	ASSIGNED BY	GENE NAME	DATE\n")
...
@pytest.mark.parametrize("row_length,expected_fields", [
    (0, 0),   # Empty row
    (5, 5),   # Partial row
    (16, 16), # Full row
...
UniProtKB	Q12345	TEST		GO:0009999	test location	CC	ECO:0007322	IEA	PMID:99999		NCBITaxon:9606	Homo sapiens	UniProt	Test protein	20180515"""
    ...
    try:
        ...
        added_count, output_path, refs_added = validator.seed_missing_annotations(yaml_path, goa_path)
